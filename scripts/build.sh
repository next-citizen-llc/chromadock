...
swiftc -O \
  -sdk "$SDK" \
  -target "$TARGET" \
  -framework SwiftUI -framework AppKit -framework ServiceManagement -framework ImageIO -framework UniformTypeIdentifiers -framework WebKit -framework CoreImage \
